    ...
    separator_(dict.lookupOrDefault<string>("separator", string(","))[0])
{
    ...
    {
      FatalErrorIn("Foam::csvTableReader<T>::csvTableReader(const dictionary& dict)")
...
namespace Foam
{
    ...
    scalar csvTableReader<scalar>::readValue(const List<string>& splitted)
    {
        ...
        {
          FatalErrorIn("Foam::csvTableReader<scalar>::readValue(const List<string>& splitted)")
    ...
    Type csvTableReader<Type>::readValue(const List<string>& splitted)
    {
        ...
        for(label i = 0;i < pTraits<Type>::nComponents; i++)
        {
            ...
            {
              FatalErrorIn("Foam::csvTableReader<T>::readValue(const List<string>& splitted)")
...
{
  FatalErrorIn("Foam::csvTableReader<Type>::operator()") << "Not implemented" << Foam::abort(FatalError);
}
