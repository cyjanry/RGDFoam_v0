...
) const
{
    ...
    scalar maxLimit = data[label(data.size() - 1)].first();
    if (n == 1)
    ...
    else if (lookupValue > maxLimit)
    {
        switch (boundsHandling_)
        {
		 ...
		 Type y2 = data[data.size()-2].second();
                return y1 + (lookupValue - x1)/(x2-x1) * (y2 - y1); //extrapolation
                return data[label(data.size() - 1)].second();
...
) const
{
    ...
    label nY = (this->operator[](0)).second().size();
    if(nY == 1)
    ...
    else if (nX == 1)
    {
        ...
        //return extrapolateValue(t.first().second(), valueY);
         return extrapolateValue(t[0].second(), valueY);
    }
    else
    {
	...
	*/
	...
	if(nY == 1)
	  //return row0.first().second() + factor * (row1.first().second() - row0.first().second());
          return row0[0].second() + factor * (row1[0].second() - row0[0].second());
	/*}
...
//	  ));
    ...
    points.setSize(label(points.size() + 1));
    points[points.size() - 1] = ve;
