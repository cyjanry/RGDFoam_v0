...
void Foam::heTabularThermo<BasicTabularThermo, MixtureType>::calculate()
{
	...
	TCells[celli] = this->TTable(pCells[celli], hCells[celli] + pCells[celli] / mixture_.rho(pCells[celli], TCells[celli]));
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        else
        {
            forAll(pT, facei)
            {
                const typename MixtureType::thermoType& mixture_ =
                    this->patchFaceMixture(patchi, facei);
		pT[facei] = this->TTable(pp[facei], ph[facei] + pp[facei] / mixture_.rho(pp[facei], pT[facei]));
...
:
    hPsiThermo<MixtureType>(mesh, phaseName)
{
