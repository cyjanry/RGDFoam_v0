...
void Foam::AUSMplusALERealFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
        ...
        const scalar aRight = max(SMALL, ARight);//Foam::sqrt(max(SMALL,kappaRight*pRight/max(rhoRight,rhoMin)));

        // DensityTotalEnergy
        const scalar rhoHLeft =
            rhoLeft*hLeft + rhoLeft *(0.5*magSqr(ULeft) +kLeft);
            //pLeft *kappaLeft /(kappaLeft -1.0)+rhoLeft *(0.5*magSqr(ULeft) +kLeft);
        const scalar rhoHRight =
            rhoRight*hRight + rhoRight*(0.5*magSqr(URight)+kRight);
