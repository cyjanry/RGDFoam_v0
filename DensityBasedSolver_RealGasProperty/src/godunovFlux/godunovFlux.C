...
)
{
    ...
    if (multidimLimiterSwitch_)
    {
        ...
        //Info << "here before phi min store prev iter" << endl;
        ...
        //Info << "here is before phi min correct boudnary" << endl;
...
//             }
    ...
    //Info << "here is before phi limiter correct boudnary condition" << endl;
...
// //
            ...
            //A_[own],
            //A_[nei],
            //h_[own],//kappa_[own],       // left kappa
            //h_[nei],//kappa_[nei],       // right kappa
    ...
    forAll(p_.boundaryField(), patchi)
    {
        ...
        const fvPatch& curPatch = p_.boundaryField()[patchi].patch();// add current patch
...
/*            const scalarField ppLeft  =
            ...
            const scalarField pkRight =
               k_.boundaryField()[patchi].patchNeighbourField();

            const scalarField pkappaLeft  =
               kappa_.boundaryField()[patchi].patchInternalField();
            const scalarField pkappaRight =
               kappa_.boundaryField()[patchi].patchNeighbourField();
*/
...
//             const vectorField deltaRLeftField = pp.patch().delta();
            ...
            forAll(pp, faceI)
            {
                ...
                //Info << "delataRleft =" << deltaRLeft << " and pDeltaRLeft =" << pDeltaRLeft[faceI] << endl;
                //vector deltaRRight = faceCenter[faceI] - pCellCenterRight[faceI];

                //if(mag(deltaRLeft - pDeltaRLeft[faceI])!=0)
               // {
               //    Info<< "faceI =" << faceI << " the deltaRLeft =" << deltaRLeft << " and pDeltaRLeft =" << pDeltaRLeft[faceI] << endl;
               // }

                //if(mag(deltaRRight - pDeltaRRight[faceI])!=0)
               // {
               //    Info<< "faceI =" << faceI << " the deltaRRight =" << deltaRRight << " and pDeltaRRight =" << pDeltaRRight[faceI] << endl;
               // }
