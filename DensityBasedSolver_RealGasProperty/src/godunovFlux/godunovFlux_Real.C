...
void Foam::godunovFlux_Real<Flux>::update(Switch secondOrder)
{
    ...
    //const volScalarField h_("h",thermophysicalModel_.h());   //Add a copy here. Jianhui Qi 20-06-2017
...
//         const fvsPatchVectorField& pFaceCenter = faceCenter.boundaryField()[patchi];
        ...
        if (pp.coupled())
        {
            ...
            const scalarField pALeft  = pA.patchInternalField();
            const scalarField pARight = pA.patchNeighbourField();            

            const scalarField phLeft  = ph.patchInternalField();
            const scalarField phRight = ph.patchNeighbourField();
